...
public class UserAuthTokenDao {
    ...
    public UserAuthTokenEntity getUserAuthTokenByUser(final UserEntity user) {
        try {
            return entityManager.createNamedQuery("userAuthTokenByUserId",UserAuthTokenEntity.class).setParameter("user" , user).getSingleResult();
        } catch(NoResultException e) {
            return null;
        }
    }
